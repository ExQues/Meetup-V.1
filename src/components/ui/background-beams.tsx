...
export const BackgroundBeams = React.memo(({ className, color = "#18CCFC", visible = 18, randomize = true }: { className?: string; color?: string; visible?: number; randomize?: boolean }) => {
  const paths = [
  ...
  const filtered = useMemo(() => {
    ...
    const count = Math.max(1, Math.min(visible, len))
    if (!randomize) {
      const step = Math.max(1, Math.ceil(len / count))
      return paths.filter((_, i) => i % step === 0)
    }
    const picked = new Set<number>()
    while (picked.size < count) {
      picked.add(Math.floor(Math.random() * len))
    }
  ...
  return (
    <div
      className={cn(
        "absolute inset-0 h-full w-full [mask-size:40px] [mask-repeat:no-repeat] flex items-center justify-center",
        className,
      )}
    >
      <svg
      ...
      >
          ...
          {filtered.map((path, index) => (
            <motion.linearGradient
              ...
              animate={{ x1: ["0%", "100%"], x2: ["0%", "95%"], y1: ["0%", "100%"], y2: ["0%", "98%"] }}
              transition={{ duration: 12 + Math.random() * 6, ease: "easeInOut", repeat: Infinity }}
            >
              <stop stopColor={color} stopOpacity="0"></stop>
              <stop stopColor={color}></stop>
              <stop offset="60%" stopColor={color}></stop>
